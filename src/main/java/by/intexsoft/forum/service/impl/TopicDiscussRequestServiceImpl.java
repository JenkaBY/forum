...
public class TopicDiscussRequestServiceImpl extends AbstractEntityServiceImpl<TopicDiscussRequest> implements TopicDiscussRequestService {
    ...
    @Autowired
    public TopicDiscussRequestServiceImpl(TopicDiscussRequestRepository repository, TopicService topicService) {
        super(repository);
     ...
     */
    @Override
    public Page<TopicDiscussRequest> findAllPending(long topicId, Pageable pageable) {
        Topic topic = topicService.find(topicId);
        return ((TopicDiscussRequestRepository) repository).findByStatusAndTopic(Status.PENDING, topic, pageable);
    }
